...
net_vanna: float, delta_iv: float,
):
“””
Vanna-Adjusted Gamma-Theta Breakeven Range.

```
Extends Park & Zhao (2024) GTBR by incorporating the volatility
dimension via Vanna, following Carr & Wu (2020) full PnL attribution.

Theory:
  Standard GTBR (Park & Zhao eq.1):
    Daily PnL = θ/365 + 50·Γ·r²
    GTBR = ± σ/√365   (as % move)

  Full PnL with Vanna (Carr & Wu 2020 Appendix A1):
    Daily PnL = θ/365 + 50·Γ·r² + Vanna·(ΔF)·(Δσ)

  Setting PnL = 0 and expressing in price space:
    50·Γ·(ΔF)² + Vanna·(ΔF)·(Δσ) + θ/365 = 0

  This is a quadratic in ΔF:
    a = 50·Γ     (dollar gamma coefficient)
    b = Vanna·Δσ (vanna × expected IV change)
    c = θ/365    (daily theta)

  Solutions:  ΔF = [−b ± √(b²−4ac)] / (2a)

  The key insight: when Δσ ≠ 0, the two roots are NOT symmetric
  around zero — the range SHIFTS in the direction of Vanna flow.

Practical meaning for Gold Futures:
  - IV rising (Δσ > 0):  downside range widens, upside narrows
    → Dealer forced to sell MORE when price drops (Gamma + Vanna)
    → This is the "Hidden Killer" from Mtraders analysis

  - IV falling (Δσ < 0):  upside range widens, downside narrows
    → Dealer unwinds delta more aggressively on rallies
...
dte      : days to expiry
net_vanna: aggregate Vanna × OI across strikes (from GEX calc)
           positive = net long vanna, negative = net short vanna
delta_iv : expected daily IV change (decimal), e.g. +0.01 = +1% IV
           use IV skew slope or recent IV change as proxy

Returns
-------
(lo_daily, hi_daily)     — Vanna-adjusted daily breakeven range
(lo_expiry, hi_expiry)   — Vanna-adjusted expiry breakeven range
vanna_shift_daily        — asymmetric shift from Vanna (daily)
vanna_shift_expiry       — asymmetric shift from Vanna (expiry)
"""
T = max(dte / 365.0, 1e-10)

# ── ATM Greeks (r=0, Black-76) ──
gamma_atm = _b76_gamma(F, F, T, atm_iv)       # Γ at ATM
theta_atm = _b76_theta_atm(F, atm_iv, T)      # θ at ATM (annualised)
dollar_gamma = gamma_atm * F * F / 100.0       # $Γ = Γ·F²/100

if dollar_gamma == 0 or gamma_atm == 0:
    # Fallback to standard GTBR
...
# ── Quadratic coefficients for ΔF ──
# PnL = a·(ΔF)² + b·(ΔF) + c = 0
# where:
#   a = ½ · gamma_atm  (in ΔF space, not % space)
#   b = net_vanna · delta_iv  (Vanna × Δσ)
#   c = theta_daily

# --- Daily (Δt = 1/365) ---
theta_daily = theta_atm / 365.0
a_d = 0.5 * gamma_atm
b_d = net_vanna * delta_iv        # Vanna flow from IV change
c_d = theta_daily

disc_d = b_d * b_d - 4.0 * a_d * c_d

if disc_d < 0 or a_d == 0:
    # No real roots → fallback to standard
    d_day = F * atm_iv / math.sqrt(365.0)
    ...
    # Shift = how much the midpoint moved from ATM
    shift_d = (lo_d + hi_d) / 2.0 - F

# --- Expiry (Δt = T) ---
theta_expiry = theta_atm * T      # total theta over remaining life
# Scale Vanna effect by √DTE (IV change accumulates over time)
b_e = net_vanna * delta_iv * math.sqrt(dte) if dte > 0 else 0.0
a_e = 0.5 * gamma_atm
c_e = theta_expiry
            ...
            if not _atm or not _dte:
                ...
                if _cdf.empty:
                    ...
                    if _iv_comp and _dte > 0:
                        ...
                        # Compute aggregate Vanna from OI data
                        ...
                        _delta_iv = 0.0
                        if _iv_v and _iv_o:
                            # Use OI-Vol IV difference as proxy
                            # for expected IV movement direction
                    ...
                    # ── Vanna-Adjusted metrics row ──
                    v1, v2, v3, v4 = st.columns(4)
                    with v1:
                        st.metric(
                            "🔷 Net Vanna",
                            f"{_net_vanna:,.2f}",
                            help="Σ Vanna × (Call−Put) OI across all strikes",
                        )
                    with v2:
                        _div_str = (
                            f"{_delta_iv*100:+.2f}%"
                    ...
                    if _net_vanna != 0 and _delta_iv != 0:
                        ...
                        _vanna_msg = (
                            f"Vanna flow shifts GTBR toward **{_vanna_dir}** "
                            f"(Vanna={'+'if _net_vanna>0 else ''}{_net_vanna:.2f} × "
                            f"ΔIV={_delta_iv*100:+.2f}%) — "
                        )
                        if _vanna_dir == "downside":
                            _vanna_msg += (
                                "dealers face amplified selling pressure "
                                "when price drops (Gamma + Vanna compound)"
                            )
                        else:
                            _vanna_msg += (
                                "dealers face amplified buying pressure "
                                "when price rises (IV drop releases delta)"
                            )
                        st.caption(f"🔷 **Vanna Effect:** {_vanna_msg}")
