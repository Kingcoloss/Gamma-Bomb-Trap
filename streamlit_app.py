...
def _b76_vanna(F: float, K: float, T: float, sigma: float) -> float:
    ...
    Theory (from project papers):
      Mtraders Gold Analysis: "Vanna คือเชื้อเพลิงลับที่ทำให้ตลาด Crash
      แรงกว่าทฤษฎี — Dealer โดนบีบจากทั้งราคาที่ลง (Gamma) และ
      ความกลัวที่พุ่งขึ้น (Vanna)"
...
def _b76_theta_atm(F: float, sigma: float, T: float) -> float:
    """
    ...
    if T <= 0 or sigma <= 0 or F <= 0:
        return 0.0
    return -F * sigma * _norm_pdf(0.0) / (2.0 * math.sqrt(T))


def _b76_volga(F: float, K: float, T: float, sigma: float) -> float:
    """
    ...
    try:
        d1 = _b76_d1(F, K, T, sigma)
        d2 = d1 - sigma * math.sqrt(T)
        vega = F * _norm_pdf(d1) * math.sqrt(T)
...
def calculate_vanna_adjusted_gtbr(
    ...
    Note: The cross-derivative (Vanna) carries NO ½ coefficient in the
    Taylor expansion — only pure second-order terms (Gamma, Volga) do.

    For a delta-hedged portfolio (Delta term = 0), setting PnL = 0
    over one day (dt = 1/365):
    ...
    where:
      a = ½ · Γ                           (gamma coefficient)
      b = Vanna · Δσ                      (cross-derivative, no ½)
      c = θ/365 + ½ · Volga · (Δσ)²      (theta + volga contribution)

    Note: Volga term shifts the CONSTANT term (c), making breakeven
    easier or harder to reach depending on Volga sign:
      - Short Volga + IV spike: c becomes more negative → wider range
        (the "Shadow Vega" from Part 4 paper — exponential loss in IV spike)
      - Long Volga + IV spike: c becomes more positive → narrower range

    Solutions:  ΔF = [−b ± √(b² − 4ac)] / (2a)

    When Δσ = 0: reduces to standard GTBR (Park & Zhao eq.1)
    When Δσ ≠ 0: range shifts asymmetrically (Vanna) and
                  widens/narrows symmetrically (Volga)
    ...
    dte       : days to expiry
    net_vanna : aggregate Vanna × (Call−Put) OI across strikes
    net_volga : aggregate Volga × (Call+Put) OI across strikes
    delta_iv  : expected daily IV change (decimal)

    Returns
    -------
    (lo_daily, hi_daily, lo_expiry, hi_expiry, shift_daily, shift_expiry)
    """
    T = max(dte / 365.0, 1e-10)

    # ── ATM Greeks (r=0, Black-76) ──
    gamma_atm = _b76_gamma(F, F, T, atm_iv)
    theta_atm = _b76_theta_atm(F, atm_iv, T)

    if gamma_atm == 0:
        d_exp = F * atm_iv * math.sqrt(T)
        d_day = F * atm_iv / math.sqrt(365.0)
        return (F - d_day, F + d_day, F - d_exp, F + d_exp, 0.0, 0.0)
    ...
    a_d = 0.5 * gamma_atm
    b_d = net_vanna * delta_iv                 # ← cross-term: no ½ coefficient
    c_d = theta_daily + volga_pnl_daily        # ← Volga term added
    ...
    # IV change scales with √DTE (diffusion process)
    delta_iv_expiry = delta_iv * math.sqrt(dte) if dte > 0 else 0.0
    volga_pnl_expiry = 0.5 * net_volga * delta_iv_expiry * delta_iv_expiry

    a_e = 0.5 * gamma_atm
    b_e = net_vanna * delta_iv_expiry           # ← cross-term: no ½ coefficient
    c_e = theta_expiry + volga_pnl_expiry       # ← Volga term added
    ...
    return (lo_d, hi_d, lo_e, hi_e, shift_d, shift_e)
...
def filter_session_data(df, data_type):
    ...
    #    pre-market), use the LATEST available date's session window instead
    #    so charts still render for analysis. ──
    if df_filtered.empty:
        latest_dt = df['Datetime'].max()
        fallback_date = latest_dt.date()
        # If latest data time is before 10:00, it belongs to previous day's session
        if latest_dt.hour < 10:
        ...
        fb_end = fb_start + timedelta(hours=15)
...
def fetch_github_history(file_path, max_commits=200):
    ...
    def _fetch_commits(date_filter):
        ...
        date_filter : date | None
            ...
            (newest-first traversal stops on the first older commit).
    ...
    #    fetch the latest N commits regardless of date so the chart still
    #    renders with the most recent available data. ──
...
if 'last_fetch_datetime' not in st.session_state: st.session_state.last_fetch_datetime = None

# ── Helper: extract last fetch datetime from loaded data ──
def _get_data_last_fetch_dt():
    """Return the max Datetime from intraday data as the last-fetch marker."""
    if 'my_intraday_data' in st.session_state and not st.session_state.my_intraday_data.empty:
        return st.session_state.my_intraday_data['Datetime'].max()
    return None

# ── Compute current session start (Bangkok timezone) ──
...
_current_session_start = pd.Timestamp(
    year=_current_session_date.year,
    month=_current_session_date.month,
    day=_current_session_date.day,
    hour=10, minute=0, second=0,
    tz='Asia/Bangkok',
)

# ── Force re-fetch when last_fetch_datetime is older than the current
#    session start (10:00 Bangkok).  Comparing full datetime instead of
#    just the date portion correctly detects staleness both across
#    overnight rollovers AND within-session edge cases (e.g. data
#    fetched at 09:xx before the new session opened). ──
_data_stale = (
    st.session_state.last_fetch_datetime is not None
    and hasattr(st.session_state.last_fetch_datetime, 'date')
    and st.session_state.last_fetch_datetime < _current_session_start
)

if _data_stale and ('my_intraday_data' in st.session_state and 'my_oi_data' in st.session_state):
    # Re-fetch both datasets on date rollover
    ...
    st.session_state.last_fetch_datetime = _get_data_last_fetch_dt()
    st.session_state.sha_intra = None
    st.session_state.sha_oi    = None
    st.session_state.last_auto_check = 0.0

# ── Initial load: fetch each dataset independently if missing or empty ──
_need_intra = ('my_intraday_data' not in st.session_state or st.session_state.my_intraday_data.empty)
_need_oi    = ('my_oi_data' not in st.session_state or st.session_state.my_oi_data.empty)

if _need_intra or _need_oi:
    if _need_intra:
    ...
    st.session_state.last_fetch_datetime = _get_data_last_fetch_dt()
    print(f"last_fetch_dt={st.session_state.last_fetch_datetime}")
                ...
                if not _atm or not _dte:
                    ...
                    if _cdf.empty:
                        ...
                        if _iv_comp and _dte > 0:
                            ...
                            _delta_iv = 0.0
                            if _iv_v and _iv_o:
                                _delta_iv = (_iv_v - _iv_o)

                            # Vanna + Volga Adjusted GTBR
                            (
                                _va_ld, _va_hd,
                                _va_le, _va_he,
                                _v_shift_d, _v_shift_e,
                        ...
                        with v3:
                            ...
                            st.metric(
                                "🔷 ΔIV proxy",
                                _div_str,
                                help="IV_intraday − IV_OI as proxy for expected IV move",
                            )
                        with v4:
                            if _va_ld and _va_hd:
                                st.metric(
...
""")
        ...
        st.caption(
            ...
            "Bollen, N. & Whaley, R. (2004) *\"Does Net Buying Pressure Affect the Shape of Implied Volatility Functions?\"* J. Finance  ·  "
            "Barbon, A. & Buraschi, A. (2021) *\"Gamma Fragility\"* J. Financial Economics"
        )
...
""")
        ...
        st.caption(
            "📚 Silic, M. & Poulsen, R. (2021) *\"Gamma Exposure and the Dynamics of Delta Hedging\"* — eq.27-28  ·  "
            "Bossu, S. et al. (2005) *\"Variance Swaps and Gamma Exposure\"* — GTBR derivation  ·  "
            "Park, H. & Zhao, L. (2020) *\"Gamma Scalping Breakeven\"* — eq.1"
        )
...
""")
        ...
        st.caption(
            "📚 Carr, P. & Wu, L. (2020) *\"Static Hedging of Standard Options\"* J. Financial Econometrics — full PnL attribution  ·  "
...
""")
        ...
        st.caption(
            "📚 Carr, P. & Wu, L. (2020) *\"Static Hedging of Standard Options\"* — Appendix A1, full Taylor expansion  ·  "
            "Castagna, A. & Mercurio, F. (2007) *\"The Vanna-Volga Method\"* — quadratic adjustment framework  ·  "
            "Alexander, C. (2008) *\"Market Risk Analysis Vol.III\"* Ch.6, Wiley — Taylor PnL decomposition"
        )
...
""")
        ...
        st.caption(
            "📚 Barbon, A. & Buraschi, A. (2021) *\"Gamma Fragility\"* — OI + Volume composite approach  ·  "
            "Kolanovic, M. & Krishnamachari, R. (2017) *\"Big Data and AI Strategies\"* J.P. Morgan — multi-signal dealer hedging model"
        )
...
""")
        ...
        st.caption(
            "📚 Hedged Capital (2023) *\"Gamma Exposure (GEX) — Dealer Hedging Playbook\"*  ·  "
            "Sinclair, E. (2013) *\"Volatility Trading\"* 2nd Ed., Wiley — Ch.7 Gamma Scalping strategies  ·  "
