...
`Γ_ATM` และ `θ_ATM` ใช้ค่า ATM เพราะ ATM Strike ครอบงำ Breakeven Condition
`Net_Vanna` และ `Net_Volga` ใช้ค่าสะสมทุก Strike เพราะ Vol Sensitivity มาจากทั้ง Book
""")
...
**เกณฑ์ Dashboard**
```
Block = |γ-Flow (Vol)| / |GEX (OI)| ≥ 2.0x
```
ถ้า Volume Flow เป็น 2 เท่าของ OI ที่ Strike นั้น → ผิดปกติ → น่าจะเป็น Block Trade
